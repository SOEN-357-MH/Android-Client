...
class HomeViewModel @Inject constructor(
    ...
    private val _getTrendingMoviesByPageResponse =
        MutableStateFlow<GetTrendingMoviesByPageEvent>(GetTrendingMoviesByPageEvent.Empty)
    val getTrendingMoviesByPageResponse: StateFlow<GetTrendingMoviesByPageEvent> =
        _getTrendingMoviesByPageResponse

    private val _getGenreMoviesByPageResponse =
        MutableStateFlow<GetGenreMoviesByPageEvent>(GetGenreMoviesByPageEvent.Empty)
    val getGenreMoviesByPageResponse: StateFlow<GetGenreMoviesByPageEvent> =
        _getGenreMoviesByPageResponse

    private val _getGenreShowsByPageResponse =
        MutableStateFlow<GetGenreShowsByPageEvent>(GetGenreShowsByPageEvent.Empty)
    val getGenreShowsByPageResponse: StateFlow<GetGenreShowsByPageEvent> =
        _getGenreShowsByPageResponse

    private val _getTrendingShowsByPageResponse =
        MutableStateFlow<GetTrendingShowsByPageEvent>(GetTrendingShowsByPageEvent.Empty)
    val getTrendingShowsByPageResponse: StateFlow<GetTrendingShowsByPageEvent> =
        _getTrendingShowsByPageResponse

    private val _getBaseImageUrlResponse =
        MutableStateFlow<GetBaseImageUrlEvent>(GetBaseImageUrlEvent.Empty)
    val getBaseImageUrlResponse: StateFlow<GetBaseImageUrlEvent> = _getBaseImageUrlResponse

    private val _getImageSizeResponse =
        MutableStateFlow<GetImageSizesEvent>(GetImageSizesEvent.Empty)
    val getImageSizeResponse: StateFlow<GetImageSizesEvent> = _getImageSizeResponse

    private val _getMovieGenresResponse =
        MutableStateFlow<GetMovieGenresEvent>(GetMovieGenresEvent.Empty)
    val getMovieGenresResponse: StateFlow<GetMovieGenresEvent> = _getMovieGenresResponse

    private val _getMovieProvidersResponse =
        MutableStateFlow<GetMovieProvidersEvent>(GetMovieProvidersEvent.Empty)
    val getMovieProvidersResponse: StateFlow<GetMovieProvidersEvent> = _getMovieProvidersResponse

    private val _getShowProvidersResponse =
        MutableStateFlow<GetShowProvidersEvent>(GetShowProvidersEvent.Empty)
    val getShowProvidersResponse: StateFlow<GetShowProvidersEvent> = _getShowProvidersResponse

    private val _discoverMovieResponse =
        MutableStateFlow<DiscoverMovieEvent>(DiscoverMovieEvent.Empty)
    val discoverMovieResponse: StateFlow<DiscoverMovieEvent> = _discoverMovieResponse

    private val _addMovieToWatchlistResponse =
        MutableStateFlow<AddMovieToWatchlistEvent>(AddMovieToWatchlistEvent.Empty)
    val addMovieToWatchlistResponse: StateFlow<AddMovieToWatchlistEvent> =
        _addMovieToWatchlistResponse

    private val _addShowToWatchlistResponse =
        MutableStateFlow<AddShowToWatchlistEvent>(AddShowToWatchlistEvent.Empty)
    val addShowToWatchlistResponse: StateFlow<AddShowToWatchlistEvent> = _addShowToWatchlistResponse

    private val _removeMovieFromWatchlistResponse =
        MutableStateFlow<RemoveMovieFromWatchlistEvent>(RemoveMovieFromWatchlistEvent.Empty)
    val removeMovieFromWatchlistResponse: StateFlow<RemoveMovieFromWatchlistEvent> =
        _removeMovieFromWatchlistResponse

    private val _removeShowFromWatchlistResponse =
        MutableStateFlow<RemoveShowFromWatchlistEvent>(RemoveShowFromWatchlistEvent.Empty)
    val removeShowFromWatchlistResponse: StateFlow<RemoveShowFromWatchlistEvent> =
        _removeShowFromWatchlistResponse

    private val _getMovieWatchlistResponse =
        MutableStateFlow<GetMovieWatchlistEvent>(GetMovieWatchlistEvent.Empty)
    val getMovieWatchlistResponse: StateFlow<GetMovieWatchlistEvent> = _getMovieWatchlistResponse

    private val _getShowWatchlistResponse =
        MutableStateFlow<GetShowWatchlistEvent>(GetShowWatchlistEvent.Empty)
    ...
    var listOfGenresMovies: ConcurrentHashMap<String, MutableList<MediaBody>> = ConcurrentHashMap()
    var listOfGenresShows: ConcurrentHashMap<String, MutableList<MediaBody>> = ConcurrentHashMap()
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.addMovieToWatchlist(username, movieID)) {
                    is Resource.Success -> {
                        _addMovieToWatchlistResponse.value =
                            AddMovieToWatchlistEvent.Success("Movie Added to Watchlist")
                    }
                    is Resource.Error -> _addMovieToWatchlistResponse.value =
                        AddMovieToWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _addMovieToWatchlistResponse.value =
                        AddMovieToWatchlistEvent.Exception(response.message!!)
                }
            } else _addMovieToWatchlistResponse.value =
                AddMovieToWatchlistEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.addShowToWatchlist(username, showID)) {
                    is Resource.Success -> {
                        _addShowToWatchlistResponse.value =
                            AddShowToWatchlistEvent.Success("Show Added to Watchlist")
                    }
                    is Resource.Error -> _addShowToWatchlistResponse.value =
                        AddShowToWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _addShowToWatchlistResponse.value =
                        AddShowToWatchlistEvent.Exception(response.message!!)
                }
            } else _addShowToWatchlistResponse.value =
                AddShowToWatchlistEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.removeMovieFromWatchlist(username, movieID)) {
                    is Resource.Success -> {
                        _removeMovieFromWatchlistResponse.value =
                            RemoveMovieFromWatchlistEvent.Success("Movie Removed from Watchlist")
                    }
                    is Resource.Error -> _removeMovieFromWatchlistResponse.value =
                        RemoveMovieFromWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _removeMovieFromWatchlistResponse.value =
                        RemoveMovieFromWatchlistEvent.Exception(response.message!!)
                }
            } else _removeMovieFromWatchlistResponse.value =
                RemoveMovieFromWatchlistEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.removeShowFromWatchlist(username, showID)) {
                    is Resource.Success -> {
                        _removeShowFromWatchlistResponse.value =
                            RemoveShowFromWatchlistEvent.Success("Show Removed from Watchlist")
                    }
                    is Resource.Error -> _removeShowFromWatchlistResponse.value =
                        RemoveShowFromWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _removeShowFromWatchlistResponse.value =
                        RemoveShowFromWatchlistEvent.Exception(response.message!!)
                }
            } else _removeShowFromWatchlistResponse.value =
                RemoveShowFromWatchlistEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getTrendingMoviesByPage(page)) {
                    is Resource.Success -> {
                        ...
                        _getTrendingMoviesByPageResponse.value =
                            GetTrendingMoviesByPageEvent.Success("Trending Movies Retrieved")
                    }
                    is Resource.Error -> _getTrendingMoviesByPageResponse.value =
                        GetTrendingMoviesByPageEvent.Failure(response.message!!)
                    is Resource.Exception -> _getTrendingMoviesByPageResponse.value =
                        GetTrendingMoviesByPageEvent.Exception(response.message!!)
                }
            } else _getTrendingMoviesByPageResponse.value =
                GetTrendingMoviesByPageEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getGenreMoviesByPage(page, genre)) {
                    is Resource.Success -> {
                        ...
                        _getGenreMoviesByPageResponse.value =
                            GetGenreMoviesByPageEvent.Success("Genre Movies Retrieved")
                    }
                    is Resource.Error -> _getGenreMoviesByPageResponse.value =
                        GetGenreMoviesByPageEvent.Failure(response.message!!)
                    is Resource.Exception -> _getGenreMoviesByPageResponse.value =
                        GetGenreMoviesByPageEvent.Exception(response.message!!)
                }
            } else _getGenreMoviesByPageResponse.value =
                GetGenreMoviesByPageEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getGenreShowsByPage(page, genre)) {
                    is Resource.Success -> {
                        ...
                        _getGenreShowsByPageResponse.value =
                            GetGenreShowsByPageEvent.Success("Genre Shows Retrieved")
                    }
                    is Resource.Error -> _getGenreShowsByPageResponse.value =
                        GetGenreShowsByPageEvent.Failure(response.message!!)
                    is Resource.Exception -> _getGenreShowsByPageResponse.value =
                        GetGenreShowsByPageEvent.Exception(response.message!!)
                }
            } else _getGenreShowsByPageResponse.value =
                GetGenreShowsByPageEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getTrendingShowsByPage(page)) {
                    is Resource.Success -> {
                        ...
                        _getTrendingShowsByPageResponse.value =
                            GetTrendingShowsByPageEvent.Success("Trending Shows Retrieved")
                    }
                    is Resource.Error -> _getTrendingShowsByPageResponse.value =
                        GetTrendingShowsByPageEvent.Failure(response.message!!)
                    is Resource.Exception -> _getTrendingShowsByPageResponse.value =
                        GetTrendingShowsByPageEvent.Exception(response.message!!)
                }
            } else _getTrendingShowsByPageResponse.value =
                GetTrendingShowsByPageEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getBaseImageUrl()) {
                    is Resource.Success -> {
                        ...
                        _getBaseImageUrlResponse.value =
                            GetBaseImageUrlEvent.Success("Base Image Url Retrieved")
                    }
                    is Resource.Error -> _getBaseImageUrlResponse.value =
                        GetBaseImageUrlEvent.Failure(response.message!!)
                    is Resource.Exception -> _getBaseImageUrlResponse.value =
                        GetBaseImageUrlEvent.Exception(response.message!!)
                }
            } else _getBaseImageUrlResponse.value =
                GetBaseImageUrlEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getImageSizes()) {
                    ...
                    is Resource.Error -> _getImageSizeResponse.value =
                        GetImageSizesEvent.Failure(response.message!!)
                    is Resource.Exception -> _getImageSizeResponse.value =
                        GetImageSizesEvent.Exception(response.message!!)
                }
            } else _getImageSizeResponse.value =
                GetImageSizesEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getMovieGenres()) {
                    is Resource.Success -> {
                        ...
                        _getMovieGenresResponse.value =
                            GetMovieGenresEvent.Success("Movie Genres Retrieved")
                    }
                    is Resource.Error -> _getMovieGenresResponse.value =
                        GetMovieGenresEvent.Failure(response.message!!)
                    is Resource.Exception -> _getMovieGenresResponse.value =
                        GetMovieGenresEvent.Exception(response.message!!)
                }
            } else _getMovieGenresResponse.value =
                GetMovieGenresEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getMovieProviders(movieId)) {
                    is Resource.Success -> {
                        ...
                        movieList.find { it.id == movieId }?.providers?.results?.let {
                            it.CA?.flatrate?.let { providers ->
                                for (provider in providers) {
                                    provider.logo_path =
                                        "${baseImageUrl}${imageSizes[6]}${provider.logo_path}"
                                }
                            }
                        }

                        for ((_, value) in listOfGenresMovies) {
                        ...
                        _getMovieProvidersResponse.value =
                            GetMovieProvidersEvent.Success("Movie Providers Retrieved")
                    }
                    is Resource.Error -> _getMovieProvidersResponse.value =
                        GetMovieProvidersEvent.Failure(response.message!!)
                    is Resource.Exception -> _getMovieProvidersResponse.value =
                        GetMovieProvidersEvent.Exception(response.message!!)
                }
            } else _getMovieProvidersResponse.value =
                GetMovieProvidersEvent.Failure("No internet connection")
        }
    }
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getShowProviders(showId)) {
                    is Resource.Success -> {
                        ...
                        _getShowProvidersResponse.value =
                            GetShowProvidersEvent.Success("Show Providers Retrieved")
                    }
                    is Resource.Error -> _getShowProvidersResponse.value =
                        GetShowProvidersEvent.Failure(response.message!!)
                    is Resource.Exception -> _getShowProvidersResponse.value =
                        GetShowProvidersEvent.Exception(response.message!!)
                }
            } else _getShowProvidersResponse.value =
                GetShowProvidersEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = repository.getMoviesBasedOnFilter(page, providers, genres)) {
                    is Resource.Success -> {
                        _discoverMovieResponse.value =
                            DiscoverMovieEvent.Success("Discovered Movies")
                    }
                    is Resource.Error -> _discoverMovieResponse.value =
                        DiscoverMovieEvent.Failure(response.message!!)
                    is Resource.Exception -> _discoverMovieResponse.value =
                        DiscoverMovieEvent.Exception(response.message!!)
                }
            } else _discoverMovieResponse.value =
                DiscoverMovieEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.getMovieWatchlist(username)) {
                    ...
                    is Resource.Error -> _getMovieWatchlistResponse.value =
                        GetMovieWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _getMovieWatchlistResponse.value =
                        GetMovieWatchlistEvent.Exception(response.message!!)
                }
            } else _getMovieWatchlistResponse.value =
                GetMovieWatchlistEvent.Failure("No internet connection")
        }
    }
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            if (networkHelper.isNetworkConnected()) {
                when (val response = userRepository.getShowWatchlist(username)) {
                    ...
                    is Resource.Error -> _getShowWatchlistResponse.value =
                        GetShowWatchlistEvent.Failure(response.message!!)
                    is Resource.Exception -> _getShowWatchlistResponse.value =
                        GetShowWatchlistEvent.Exception(response.message!!)
                }
